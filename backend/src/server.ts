...
app.post('/api/auth/logout',async(req,res,next)=>{try{const token=lerCookie(req,'agrosys_session');if(token)await pool.execute('DELETE FROM sessoes WHERE token_hash=?',[hashToken(token)]);res.setHeader('Set-Cookie','agrosys_session=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0');res.status(204).end();}catch(erro){next(erro);}});
